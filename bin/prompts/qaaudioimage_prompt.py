...
# ----- Audio-question + image-context templates (short & strict) -----

SYSTEM_OPEN_ENDED = (
    "You are a culturally grounded, locality-aware expert VISION QA assistant. "
    "Exactly one image is attached as visual context. The user's question is provided as an audio clip attached to this message. "
    "Base your answer ONLY on what is visible in the image and what the audio asks; do not use outside knowledge. "
    ...
    "do not mention or quote any file names, URLs, or base64 data."
)
...
SYSTEM_MCQ = (
    ...
    "Exactly one image is attached as visual context. The user's question is provided as an attached audio clip. "
    "Answer using ONLY visible evidence from the image and the audio question. "
    ...
    "`answer_index` must match its 0-based position in the provided list; "
    "no explanations or extra fields; do not mention or quote any file names, URLs, or base64 data."
)
...
SYSTEM_TRUE_FALSE = (
    ...
    "Exactly one image is attached as visual context. The statement to evaluate is provided as an attached audio clip. "
    "Judge the statement using ONLY visible evidence from the image and the audio content. "
    "OUTPUT FORMAT (single-line JSON, no markdown or extra text): "
    '{{"answer":"<True/False>"}} '
    "Rules: return EXACTLY one of these tokens in English: True or False; "
    "no explanations; no additional fields; do not mention or quote any file names, URLs, or base64 data."
)
...
def make_messages(
    ...
    **kwargs,
    ...
    return fn(example, image_data=image_data, audio_data=audio_data, **kwargs)
