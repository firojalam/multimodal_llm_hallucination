...
class ImageBatchProcessor:
    ...
    def generate_payload_message(self, image_path, description, question, answer):
        """
        ...
        # system_prompt = f"""You are a data augmentation writer for visual question answering.
        # Your job is to craft one hallucinatory question: a question that sounds plausible for the content of the image,
        # but is factually wrong given the supplied image description.

        # Rules:
        # - Stay on topic & structure: keep the same general type as the original (color, count, attribute,
        # spatial relation, reading text, action), but change the focus so it becomes incorrect.
        # - Hallucinate exactly one detail not in the image: introduce a new object/attribute/text/
        # relationship/number that is absent or contradicted by the description.
        # - Do not copy the original question. No near-paraphrases beyond function words.
        # - Prefer WH-questions (“What/Which/Where/How many/What color…”) over yes/no.
        # - Single sentence, concise (≤ 18 words).
        # - No negations or meta language (don’t say “not,” “absent,” “according to the image,” etc.).
        # - Safe/neutral content only.
        # - Output format: return only the new question ending with a question mark—no explanations, labels, or extra text.

        # Hallucination ideas (choose one):
        # - Swap the object (ask about a “bus” when only cars are present).
        # - Alter an attribute (color/material/brand/jersey number) not described.
        # - Change the count to a wrong number.
        # - Invent text on a sign/label that isn’t there.
        # - Flip a spatial relation (“under the bridge” vs. “on top of the bridge”).
        # - Misstate an action/state (“running” vs. “sitting”)."""

        # system_prompt = """You are a data augmentation writer for visual question answering.

        # Goal
        # - Produce ONE hallucinatory question that sounds plausible for the image but is wrong given the Description.

        # Rules
        # - Stay on the same general question type as the original (color/count/attribute/spatial/text/action) but change the focus so it becomes incorrect.
        # - Hallucinate exactly ONE underlying detail not supported by the Description (new object/attribute/text/relationship/number).
        # - If that hallucinated detail appears multiple times in the question text, mark ALL occurrences.
        # - Do not copy or near-paraphrase the original question (beyond function words).
        # - Prefer WH-questions. Single sentence, ≤ 18 words.
        # - No negations or meta language. Safe/neutral content only.
        ...
        # Notes
        # - "spans" lists every occurrence of the SAME hallucinated detail; spans must be non-overlapping and sorted by start.
        # - Indices are 0-based and end-exclusive over the characters of "question".
        # """
        ...
        # Rules
        # - Keep the same general type as the original (color/count/attribute/spatial/text/action) but change focus so it becomes incorrect.
        # - Hallucinate exactly ONE underlying detail not supported by the Description (new object/attribute/text/relationship/number).
        # - If that detail appears multiple times in the question, wrap ALL occurrences in [[ and ]].
        # - Do not copy or near-paraphrase the original question (beyond function words).
        # - Prefer WH-questions. Single sentence, ≤ 18 words. No negations or meta language. Safe/neutral content only.
        ...
        # Notes
        # - Every item in spans must correspond to one [[...]] occurrence, in reading order.
        # - All spans refer to the SAME underlying hallucinated detail.
        # """

        # system_prompt =  """You are a data augmentation writer for VQA.

        # Goal:
        # - Produce a hallucinated version of the Original_Q by replacing exactly one or more contiguous spans.
        # - Keep the interrogative pattern AND the question type identical to Original_Q (e.g., name/color/count).
        # - The change must make the question factually wrong given the Description.

        # Output (STRICT JSON ONLY)
        # {
        # "question_marked": "<the question with [[...]] around each hallucinated fragment>",
        # "spans": [ {"text": "<the exact hallucinated fragment between [[...]]"}, ... ]
        # "orig_spans": [ {"text": "<the exact fragment between [[...]]"}, ... ]
        # }

        # Rules:
        # - Single sentence (≤ 18 words). No negations or meta language.
        # - Exactly one or more spans. 0-based, end-exclusive indices.
        # - Do not rephrase outside the span.
        # Return ONLY the JSON.
        # """

        # user_prompt = f"""Image Description: {description}
        # Original Question: {question}
        # Original Answer: {answer}

        # Produce one hallucinatory question that follows the system rules. Output only the question in JSON format.
        # """

        system_prompt = f"""
        You are a data augmentation writer for visual question answering.

        Goal:
        - Produce a hallucinated version of the Original_Q by replacing exactly ONE contiguous span.
        - Keep the interrogative pattern AND the question type identical to Original_Q (e.g., name/color/count/where/text/action).
        - The single change must make the question factually wrong given the Description (introduce one unsupported object/attribute/number/text/relation/action).
        ...
        - Exactly one or more spans. 0-based, end-exclusive indices.
        - Do not alter any token outside the single replaced span.
        - Safe/neutral content only.
        - Mark ALL occurrences of the SAME hallucinated detail and mark with [[ and ]].

        Output STRICT JSON:
        {{
        "question_marked": "<the question with [[...]] around each hallucinated fragment>",                                
        "spans": [ {{"text": "<the exact hallucinated fragment between [[...]]"}}, ... ]
        "orig_span": [ {{"text": "<the exact hallucinated fragment between [[...]]"}}, ... ]                                 
        }}
        ...
        for img_obj in image_objects:
            ...
            if self.get_image_size(file_path) <= self.image_size_limit:
                ...
                payload_message = self.generate_payload_message(
                    file_path, description, question, answer
                )
