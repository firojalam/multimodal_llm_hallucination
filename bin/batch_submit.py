...
class LLMBatchBuilder:
    ...
    def _iter_qa_items(self, rec: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        # # Open-ended
        # for i, item in enumerate(qa.get("open-ended", []) or []):
        #     out.append(
        #         {
        #             "base_id": base_id,
        #             "qa_type": "open-ended",
        #             "question_id": "open-ended",  # single item label
        #             "payload": item,
        #         }
        #     )

        # # Multiple-choice
        # for i, item in enumerate(qa.get("multiple-choice", []) or []):
        #     out.append(
        #         {
        #             "base_id": base_id,
        #             "qa_type": "multiple-choice",
        #             "question_id": "multiple-choice",  # single item label
        #             "payload": item,
        #         }
        #     )
    ...
    def create_batches(
        ...
        with open(self.input_jsonl, "r", encoding="utf-8") as f:
            for line_no, line in enumerate(f, 1):
                ...
                for it in items:
                    # Build an example object for the prompt module
                    example_for_prompt = dict(it["payload"])
                    example_for_prompt["type"] = it["qa_type"]
                    # Let the prompt module pick the right language-prefixed fields
                    if modality == "text":
                        ...
                        if lang_prefix == "en":
                            audio_path = it["payload"].get(
                                "audio_file_gen_0"
                            )  ## need to update with lang_prefix
                        else:
                            audio_path = it["payload"].get(
                                f"{lang_prefix}_audio_file_gen_0"
                            )
                        audio_data = self.encode_wav(audio_path)
                        ...
                        logging.error(f"Unknown modality: {modality}")

                    if not isinstance(messages, list):
                        raise ValueError("make_messages must return a List[dict]")
